...
function helper(preorder, p_start, p_end, inorder, i_start, i_end, map) {
  ...
  let index = map.get(rootVal) // 根节点在 inorder 中的位置
  let leftNum = index - i_start // 左子树的节点数
  root.left = helper(preorder, p_start + 1, p_start + leftNum + 1, inorder, i_start, index, map)
  root.right = helper(preorder, p_start + leftNum + 1, p_end, inorder, index + 1, i_end, map)
  return root
}
